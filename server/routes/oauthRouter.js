...
const { getToken, startSession, checkSession } = require('../controllers/oauthController');
...
router.get('/callback*', getToken, startSession, (req, res) => {
  console.log("did we reach back end callback")
  res.set("Access-Control-Allow-Origin", '*'); // update to match the domain you will make the request from
  res.setHeader("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
  res.setHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS, PUT");
  return res.redirect('http://localhost8080/homepage');
});

router.get('/checkSession', checkSession, (req, res) => {
